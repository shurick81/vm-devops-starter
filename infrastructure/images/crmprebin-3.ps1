...
$logFile = '{0}-{1}.log' -f "ReportViewer.msi",$DataStamp
$MSIArguments = @(
    "/i"
    ('"{0}"' -f "C:\Install\CRMPrerequisites\ReportViewer.msi")
    "/qn"
    "/norestart"
    "/L*v"
    $logFile
...
$MSIArguments = @(
    "/i"
    ('"{0}"' -f "C:\Install\CRM\CRM2016-Server-ENU-amd64\DW\dw20sharedamd64.msi")
    "/qn"
    "/norestart"
    "/LV*"
    $logFile
